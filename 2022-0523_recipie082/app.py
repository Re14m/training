# [AWSのサービスを活用した有名人画像認識アプリの開発のレシピ ～後編(音声合成＆機械翻訳)～](https://axross-recipe.com/recipes/82)
...
class GuiApp(object):
    ...
    def make_app(self, app=None):
        ...
        self.filemenu.add_command(label='Exit', command=self.app.destroy)

        # gender
        self.gendermenu = tk.Menu(self.menubar, tearoff=0)
        self.gendermenu.add_radiobutton(label = '男性', variable = self.gendervar, value = "Male")
        self.gendermenu.add_radiobutton(label = '女性', variable = self.gendervar, value = "Female")

        # lang
        self.langmenu = tk.Menu(self.menubar, tearoff=0)
        self.langmenu.add_radiobutton(label='日本', variable=self.langvar, value='ja')
        self.langmenu.add_radiobutton(label='英語', variable=self.langvar, value='en')
        self.langmenu.add_radiobutton(label='フランス語', variable=self.langvar, value='fr')
        self.langmenu.add_radiobutton(label='ドイツ語', variable=self.langvar, value='de')
        self.langmenu.add_radiobutton(label='イタリア語', variable=self.langvar, value='it')
        self.langmenu.add_radiobutton(label='スペイン語', variable=self.langvar, value='es')

        # Add
        self.menubar.add_cascade(label='ファイル', menu=self.filemenu)
        self.menubar.add_cascade(label='性別', menu=self.gendermenu)
        self.menubar.add_cascade(label='言語', menu=self.langmenu)
    ...
    def check_filename(self):
        if self.ans_data.filename == '':  # ファイル選択しなかった場合はメイン画面へ戻る
            self.load_open = True
            messagebox.showerror('エラー', 'ファイルをもう一度選択してください')
        else:
            ...
            if ext in FILE_EXT:
                self.canvas.delete('textcanvas')
                self.canvas.delete('illust')
            ...
            if ext in FILE_EXT:             # ファイルの拡張子チェック
                ...
                messagebox.showerror('エラー', '画像ファイルではありません')
                self.fstring.set('')
    ...
    def translate(self,before_text):
        # Translate をつかうためのクライアントを作成
        translate_client = boto3.client('translate')

        # Translate_text API を利用
        response = translate_client.translate_text(Text=before_text,  
                                                SourceLanguageCode='ja',  
                                                TargetLanguageCode=self.langvar.get())
        
        # responseで返ってきたデータ
        return response['TranslatedText']

    # 解析結果を音声合成する
    def thread_polly(self):
        result_text = make_result_text(self.ans_data)

        if self.langvar.get() != 'ja':
            result_text = self.translate(result_text)

        if self.gendervar.get() == 'Male':
            polly_voice = MALE_POLLY_LIST[self.langvar.get()]
        elif self.gendervar.get() == 'Female':
            polly_voice = FEMALE_POLLY_LIST[self.langvar.get()]

        text_to_sound(result_text, polly_voice)
...
# 有名人の result_text 作成
def make_celeb_names(ans_data):
    for celeb in ans_data.celeb:
        if celeb.confidence >= CONFIDENCE:     # 一致信頼度が75%以上のものを認識結果として採用する
            ans_data.celeb_names += celeb.name +'、'

    return True

# 検出した人の数
def detect_face(ans_data):
    for celeb in ans_data.celeb:
        if celeb.confidence >= CONFIDENCE:     # 一致信頼度が75%以上のものを認識結果として採用する
            ans_data.celeb_cnt += 1
        else:                                           # 一致信頼度が75%未満はUnknown
            ans_data.unknown_cnt += 1

    ans_data.unknown_cnt += len(ans_data.unknown)

    return True

# 検出した物体の数
def detect_object(ans_data):
    for info in ans_data.object:
        if info.name not in ans_data.object_namelist:
            ans_data.object_names += info.name + '、'
            ans_data.object_namelist.append(info.name)

    return True

# 解析結果テキストを作成
def make_result_text(ans_data):
    celeb_text = 'この写真に写っている人物は、' + ans_data.celeb_names[:-1] + '　です。'
    unknown_text = '名前のわからない人が' + str(ans_data.unknown_cnt) + '名いました。'
    obejct_text = 'この写真には、' + ans_data.object_names + '　が写っています。'
    not_celeb_text = '有名人を認識できませんでした。'
    final_text = '以上、検出結果でした。'
    else_text = 'またのご利用お待ちしております'
    result_text = ''

    # celeb検出
    if ans_data.celeb_cnt > 0:
        result_text += celeb_text
    # celeb検出なし
    if ans_data.celeb_cnt <= 0:
        result_text += not_celeb_text
    # Unknown検出
    if ans_data.unknown_cnt > 0:
        result_text += unknown_text
    # Object検出
    if len(ans_data.object_namelist) > 0:
        result_text += obejct_text
    # Object検出なし and Unknown検出なし
    if len(ans_data.object_namelist) <= 0 and ans_data.unknown_cnt <= 0 and ans_data.celeb_cnt <= 0:
        result_text += else_text
    # しらんけど追加
    else:
...
# 音声データ作成
def text_to_sound(result_text, polly_voice):
    # The text string can be a maximum of 3000 bytes long.
    if len(result_text.encode()) > 3000:
    ...
    try:
        ...
        response = polly.synthesize_speech(
            Text=result_text, OutputFormat='mp3', VoiceId=polly_voice)
    except (BotoCoreError, ClientError) as error:
        # The service returned an error, exit gracefully
        print(error)
        print("Can't synthesize_speech the text")
        messagebox.showerror('Error', "BotoCoreError, ClientError: synthesize_speech the text")
        sys.exit(-1)
    ...
    if 'AudioStream' in response:
        ...
        music_path = TMPDIR.name + f'/output_{dt_now.microsecond}_{dt_now.second}_{dt_now.minute}.mp3'
        ...
        if not pygame.mixer.music.get_busy():
            ...
            mp3_length = mp3(music_path).info.length
            pygame.mixer.music.play(1)

            time.sleep(mp3_length + 0.4)
        ...
        messagebox.showerror('Error', "Could not stream audio")
        sys.exit(-1)
...
def main_proc(ans_data):
    # 解析結果をもとに有名人の氏名 and 数を検出
    detect_face(ans_data)

    # 有名人の result_text 作成
    make_celeb_names(ans_data)

    # 解析結果をもとに物体数を検出
    detect_object(ans_data)
